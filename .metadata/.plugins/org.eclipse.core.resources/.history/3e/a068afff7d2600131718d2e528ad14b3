 ...
 * @param hScale The factor by which the height is scaled (1 = original size)
 */
void drawTiles (SDL_Renderer *renderer, SDL_Texture *source, SDL_Texture *destination, float wScale, float hScale) {
	...
	for (int row = 0; row < rows; row++) {
		for (int col = 0; col < cols; col++) {
			...
			renderTexture(source, renderer, width, height, wScale, hScale);
...
int main (int argc, char **argv) {
	...
	drawTiles(renderer, background, windowTexture, SCALE_WIDTH, SCALE_HEIGHT);
